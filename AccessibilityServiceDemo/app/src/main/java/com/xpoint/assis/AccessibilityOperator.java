...
public class AccessibilityOperator {
    private static String TAG = "AccessibilityOperator";
...
//            nodeInfo = curEvent.getSource();
    ...
    private void printViews(AccessibilityNodeInfo rootInfo) {
        if (rootInfo == null || TextUtils.isEmpty(rootInfo.getClassName())) {
            return;
        } else {
            Rect rect = new Rect();
            if(rootInfo.getParent()!=null)
            {
                rootInfo.getParent().getBoundsInScreen(rect);
            }
            Log.v(TAG, "me:"+getNodeInfo(rootInfo) + "---parent:"+getNodeInfo(rootInfo.getParent())+";"+rect.width()+";"+rect.height());

            for (int i = 0; i < rootInfo.getChildCount(); i++) {
                printViews(rootInfo.getChild(i));
            }
        }
    }
    ...
    private String getNodeInfo(AccessibilityNodeInfo nodeInfo) {
        ...
        if (nodeInfo != null)
        {
            result =  nodeInfo.getClassName() + ";text:" + nodeInfo.getText() + ";id:" + nodeInfo.getViewIdResourceName() + ";";
        }
        return result;
    }

    public boolean clickTextParent(String text) {
        AccessibilityNodeInfo nodeInfo = getRootNodeInfo();
        if (nodeInfo != null) {
            return clickTextParent(nodeInfo,text);
        }
        return false;
    }

    private boolean clickTextParent(AccessibilityNodeInfo rootInfo,String text) {
        if (rootInfo != null && !TextUtils.isEmpty(rootInfo.getClassName())) {
            if ("android.widget.TextView".equals(rootInfo.getClassName().toString())) {
                if (!TextUtils.isEmpty(rootInfo.getText())&&rootInfo.getText().toString().startsWith(text)) {
                    boolean result =  performClick(rootInfo.getParent());
                    Log.v(TAG,rootInfo.getParent().getClassName().toString()+":result="+result);

                    return result;
                }
            }
            for (int i = 0; i < rootInfo.getChildCount(); i++) {
                boolean result = clickTextParent(rootInfo.getChild(i),text);
                if(result)
                {
                    return result;
                }
    ...
    private boolean performClick(AccessibilityNodeInfo targetInfo) {
