...
import com.xpoint.assis.AccessibilityLog;
import com.xpoint.assis.AccessibilityOperator;
...
public class PointService extends AccessibilityService {
    private static String TAG = "PointService";
...
//            performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK);
            ...
            String packageName = accessibilityOperator.getRootNodeInfo().getPackageName().toString();
            if(packageName.equals("com.tencent.mm"))/*微信*/
            {
                boolean result = AccessibilityOperator.getInstance().clickByText("我");
                Log.v(TAG,result+"");
            }else if(packageName.equals("com.android.browser"))
            {
                boolean result = AccessibilityOperator.getInstance().clickByText("刷新");
                Log.v(TAG,result+"");
            }else if(packageName.equals("com.android.gallery3d"))
            {
                boolean result = AccessibilityOperator.getInstance().clickByText("更多");
                Log.v(TAG,result+"");
            }else if(packageName.equals("com.ximalaya.ting.android"))/*喜马拉雅FM*/
            {
                boolean result = AccessibilityOperator.getInstance().clickById("com.ximalaya.ting.android:id/main_count_down_click_view");
                Log.v(TAG,result+"");
                result = AccessibilityOperator.getInstance().clickById("com.ximalaya.ting.android:id/host_ad_img");
                Log.v(TAG,result+"");
                result = AccessibilityOperator.getInstance().clickById("com.ximalaya.ting.android.main.application:id/main_close_ad");
                Log.v(TAG,result+"");
            }else if(packageName.equals("com.alibaba.android.rimet"))/*钉钉*/
            {
                AccessibilityOperator.getInstance().clickById("com.alibaba.android.rimet:id/btn_check_detail");
            }else if(packageName.equals("com.qiyi.video"))/*爱奇艺*/
            {
                boolean result;
                result = AccessibilityOperator.getInstance().clickByText("新说唱");
                Log.v(TAG,result+"");
                //进入首页，顶部banner位置广告
                result = AccessibilityOperator.getInstance().clickById("com.qiyi.video:id/a6w");
                Log.v(TAG,result+"");
                result = AccessibilityOperator.getInstance().clickTextParent("跳过");
                Log.v(TAG,result+"");

            }else if(packageName.equals("com.tencent.qqlive"))/*腾讯视频*/
            {
                boolean result = AccessibilityOperator.getInstance().clickByText("跳过");
                Log.v(TAG,result+"");
            }
